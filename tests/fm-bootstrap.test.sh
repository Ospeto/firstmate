...
non-string rule rationale is flagged^{"rules":[{"when":"anything","use":{"harness":"codex"},"why":false}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - rule why must be a non-empty string when present
