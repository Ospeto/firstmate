...
`bin/fm-control.sh` is the control plane: allowlisted `interrupt`, `exit`, transactional `relaunch`, and missing-endpoint `recover` verbs addressed to an exact task id, with per-harness mechanics owned by `bin/fm-control-lib.sh`, a verified postcondition per verb, and no arbitrary-text or raw-key entry point.
