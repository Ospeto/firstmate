...
Every spawn types its initial harness launch command once, confirms that the command no longer remains at the shell prompt, retries only Enter across a bounded window when submission is swallowed or cannot yet be observed, and fails the spawn if confirmation never arrives.
`FM_LAUNCH_SUBMIT_RETRIES` controls that attempt bound and defaults to `10`; `FM_LAUNCH_SUBMIT_SLEEP` controls the seconds between checks and defaults to `0.3`.
...
FM_LAUNCH_SUBMIT_RETRIES=10   # initial harness-launch confirmation attempts after typing once
FM_LAUNCH_SUBMIT_SLEEP=0.3    # seconds between initial harness-launch confirmation checks
